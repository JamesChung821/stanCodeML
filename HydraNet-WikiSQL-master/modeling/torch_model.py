...
class HydraTorch(BaseModel):
    ...
    def save_iteration(self, model_path, epoch, iteration, cur_loss):
        if "SAVE" in self.config and "DEBUG" not in self.config:
            save_path = os.path.join(model_path, "epoch_{0}_iteration_{1}_loss{2}.pt".format(epoch, iteration, cur_loss))
            if torch.cuda.device_count() > 1:
                torch.save(self.model.module.state_dict(), save_path)
            else:
                torch.save(self.model.state_dict(), save_path)
