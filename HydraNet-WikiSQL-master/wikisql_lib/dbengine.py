...
class DBEngine:
    ...
    def execute(self, table_id, select_index, aggregation_index, conditions, lower=True):
        if not table_id.startswith('table'):
            table_id = 'table_{0}'.format(table_id.replace('-', '_'))
        # print(select_index)
        # print(aggregation_index)
        # print(conditions)
        print(self.conn.query('SELECT sql from sqlite_master WHERE tbl_name = :name', name=table_id).all())
...
if __name__ == "__main__":
    engine = DBEngine("../WikiSQL/data/dev.db")
