...
if __name__ == "__main__":
    ...
    config = utils.read_conf('conf/wikisql.conf')

    output_folder = config['output_folder']

    # in_file = "data/wikigroupA_20_test.jsonl"    # wikitest
    in_file = config['test_data_path']
    out_file = f"output/{output_folder}/test_out.jsonl"
    # label_file = "WikiSQL/data/groupA_20_test.jsonl"
    label_file = config['label_file']
    db_file = "WikiSQL/data/test.db"
    model_out_file = f"output/{output_folder}/test_model_out.pkl"

    # All Best
    model_path = f"output/{output_folder}"
    epoch = config['best_epoch']  # 4
