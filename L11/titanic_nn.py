...
N1 = 10                               # This constant controls the number of neurons in Layer 1
N2 = 1                               # This constant controls the number of neurons in Layer 2


def main():
	X_train, Y = data_preprocessing()
	_, m = X_train.shape	 # Data has been transpose so x is feature, y is data number
	...
	W1, W2, B1, B2 = two_layer_nn(X, Y)	 # weights
...
def two_layer_nn(X, Y):
	...
	# Initialize W1
	W1 = np.random.rand(n, N1)-0.5	 # Build up nx3 and centralize it
	# Initialize W2
	W2 = np.random.rand(N1, N2)-0.5	 # Build up 3x1 and centralize it
	# Initialize B1
	...
	for epoch in range(NUM_EPOCHS):
		...
		# Backward Pass
		dK2 = (1/m)*np.sum(H-Y, axis=0, keepdims=True)	 # (1, m)
		dW2 = A1.dot(dK2.T)		# (1, m) with (1, m)
		dB2 = np.sum(dK2, axis=1, keepdims=True)	 # (1, m) with 1
		dA1 = W2.dot(dK2)	 # (1, m) with (3, 1) --> (3, m)
		dK1 = dA1 * np.where(K1 > 0, 1, 0)	 # (3, m)
		dW1 = X.dot(dK1.T)	 # (3,m) with (6, m) --> (6, 3)
		dB1 = np.sum(dK1, axis=1, keepdims=True)	 # (3, m)
