...
def increment(d1, scale, d2):
    ...
    for key in d2:
        d1[key] = d1.get(key, 0) + d2.get(key)*scale    # d1[key] = d1 value (0 if no key exists) + d2 value*scale
    # END_YOUR_CODE
...
def dotProduct(d1, d2):
    ...
    if len(d1) < len(d2):
        ...
        # raise Exception("Not implemented yet")
        return sum(d1[key]*d2[key] for key in d2 if key in d1)  # For loop short dictionary d2 and check if key in d1
        # END_YOUR_CODE
