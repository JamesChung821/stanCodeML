...
def main():
    ...
    x_train = data[feature_names]       # Pick up your features

    # Construct Tree
    d_tree = tree.DecisionTreeClassifier(max_depth=8, max_leaf_nodes=20, min_samples_leaf=5)
    d_tree.fit(x_train, y)
    print('Training Accuracy:', d_tree.score(x_train, y))
    tree.export_graphviz(d_tree, feature_names=feature_names, out_file='my_tree')
    # Use WebGraphviz to visualize the data

    x_test = data_preprocess(TEST_FILE, mode='test')
    x_test = x_test[feature_names]
    predictions = d_tree.predict(x_test)
    out_file(predictions, 'd_tree_titanic.csv')
